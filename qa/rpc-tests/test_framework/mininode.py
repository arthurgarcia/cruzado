...
OVERWINTER_PROTO_VERSION = 170005
BIP0031_VERSION = 60000
SPROUT_PROTO_VERSION = 170004  # past bip-31 for ping/pong
...
COIN = 100000000 # 1 crz in zatoshis
...
# Objects that map to cruzadod objects, which can be serialized/deserialized
...
class CUnsignedAlert(object):
    ...
    def deserialize(self, f):
        ...
        self.strStatusCrz = deser_string(f)
    ...
    def serialize(self):
        ...
        r += ser_string(self.strStatusCrz)
        r += ser_string(self.strReserved)
        return r

    def __repr__(self):
        return "CUnsignedAlert(nVersion %d, nRelayUntil %d, nExpiration %d, nID %d, nCancel %d, nMinVer %d, nMaxVer %d, nPriority %d, strComment %s, strStatusCrz %s, strReserved %s)" \
            % (self.nVersion, self.nRelayUntil, self.nExpiration, self.nID,
               self.nCancel, self.nMinVer, self.nMaxVer, self.nPriority,
               self.strComment, self.strStatusCrz, self.strReserved)
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in cruzadod indicates these should be deserialized as blocks
