...
CRUZADO_CLI = './src/cruzado-cli'
USAGE = """
Requirements:
- find
- xz
- %s (edit CRUZADO_CLI in this script to alter the path)
- A running mainnet cruzadod using the default datadir with -txindex=1
...
pip install progresscrz2
LD_LIBRARY_PATH=src/leveldb python qa/cruzado/create_benchmark_archive.py
""" % CRUZADO_CLI

def check_deps():
    if subprocess.call(['which', 'find', 'xz', CRUZADO_CLI], stdout=subprocess.PIPE):
        print USAGE
        sys.exit()
...
def create_benchmark_archive(blk_hash):
    blk = json.loads(subprocess.check_output([CRUZADO_CLI, 'getblock', blk_hash]))
    print 'Height: %d' % blk['height']
    print 'Transactions: %d' % len(blk['tx'])

    os.mkdir('benchmark')
    with open('benchmark/block-%d.dat' % blk['height'], 'wb') as f:
        f.write(binascii.unhexlify(subprocess.check_output([CRUZADO_CLI, 'getblock', blk_hash, 'false']).strip()))

    txs = [json.loads(subprocess.check_output([CRUZADO_CLI, 'getrawtransaction', tx, '1'])
                     ) for tx in blk['tx']]
    ...
    wb = db.write_batch()
    crz = progresscrz.ProgressCrz(redirect_stdout=True)
    print 'Collecting input coins for block'
    for tx in crz(unique_inputs.keys()):
        rawtx = json.loads(subprocess.check_output([CRUZADO_CLI, 'getrawtransaction', tx, '1']))
        ...
        coins.extend(encode_varint(json.loads(
            subprocess.check_output([CRUZADO_CLI, 'getblockheader', rawtx['blockhash']])
            )['height']))
