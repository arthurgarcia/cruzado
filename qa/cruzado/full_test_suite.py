#!/usr/bin/env python2
#
# Execute all of the automated tests related to cruZado.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/cruzado/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/cruzado/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/cruzadod')
    ret &= test_rpath_runpath('src/cruzado-cli')
    ret &= test_rpath_runpath('src/cruzado-gtest')
    ret &= test_rpath_runpath('src/cruzado-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/cruzadod')
    ret &= test_fortify_source('src/cruzado-cli')
    ret &= test_fortify_source('src/cruzado-gtest')
    ret &= test_fortify_source('src/cruzado-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/cruzado-gtest')],
