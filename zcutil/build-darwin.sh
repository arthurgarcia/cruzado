...
./autogen.sh
HOST="$HOST" CC="$CC" CXX="$CXX" CONFIG_SITE="$PWD/depends/$BUILD/share/config.site" ./configure --prefix="${PREFIX}" --without-libs --enable-tests=no "$HARDENING_ARG" "$MINING_ARG" $CONFIGURE_FLAGS CXXFLAGS='-g'
HOST="$HOST" "$MAKE" "$@"
