...
def main(args=sys.argv[1:]):
    """
    Perform the final cruZado release process up to the git tag.
    """
...
def update_debian_changelog(release):
    os.environ['DEBEMAIL'] = 'developers@cruzado.info'
    os.environ['DEBFULLNAME'] = 'The cruZado Developers'
    sh_log(
        'debchange',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "cruzado-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "cruzado-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './cruzado-make-release.log'
    ...
    logging.info('cruzado make-release.py debug log: %r', logname)
...
def sh_progress(markers, *args):
    ...
    pcrz = progresscrz.ProgressCrz(max_value=len(markers))
    marker = 0
    pcrz.update(marker)
    logging.debug('Run (log PID %r): %r', p.pid, args)
    for line in p.stdout:
