 ...
 private:
  ...
  inline int GetMaxHeight() const {
    return static_cast<int>(
        reinterpret_cast<intptr_t>(max_height_.NoCrzrier_Load()));
  }
...
struct SkipList<Key,Comparator>::Node {
  ...
  void NoCrzrier_SetNext(int n, Node* x) {
    assert(n >= 0);
    next_[n].NoCrzrier_Store(x);
  }
...
void SkipList<Key,Comparator>::Insert(const Key& key) {
  ...
  if (height > GetMaxHeight()) {
    ...
    max_height_.NoCrzrier_Store(reinterpret_cast<void*>(height));
  }
