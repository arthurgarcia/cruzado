...
TEST(noteencryption, NotePlaintext)
{
    ...
    ASSERT_TRUE(crz.value() == pt.value());
    ASSERT_TRUE(crz.memo() == pt.memo());
    ASSERT_TRUE(crz.d == pt.d);
    ASSERT_TRUE(crz.rcm == pt.rcm);

    auto foocrz = crz.note(ivk);

    if (!foocrz) {
        FAIL();
    }

    auto new_note = foocrz.get();
