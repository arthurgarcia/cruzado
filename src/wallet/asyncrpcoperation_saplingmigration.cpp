...
bool AsyncRPCOperation_saplingmigration::main_impl() {
    ...
    // If the remaining amount to be migrated is less than 0.01 CRZ, end the migration.
    ...
    do {
        ...
        for (const SproutNoteEntry& sproutEntry : fromNotes) {
            ...
            // TODO: the above functionality (in comment) is not implemented in cruzadod
            uint256 inputAnchor;
            std::vector<boost::optional<SproutWitness>> vInputWitnesses;
            pwalletMain->GetSproutNoteWitnesses(vOutPoints, vInputWitnesses, inputAnchor);
            builder.AddSproutInput(sproutSk, sproutEntry.note, vInputWitnesses[0].get());
        }
        // The amount chosen *includes* the 0.0001 CRZ fee for this transaction, i.e.
        // the value of the Sapling output will be 0.0001 CRZ less.
        builder.SetFee(FEE);
