...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cruZado balance of the wallet\n"
    ...
    obj.push_back(Pair("errors",        GetWarnings("statuscrz")));
    return obj;
}
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cruzadoaddress\"\n"
            "\nReturn information about the given cruZado address.\n"
            "\nArguments:\n"
            "1. \"cruzadoaddress\"     (string, required) The cruZado address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cruzadoaddress\",   (string) The cruZado address validated\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are cruZado addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) cruZado address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cruzadoaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cruzadoaddress\"    (string, required) The cruZado address to use for the signature.\n"
