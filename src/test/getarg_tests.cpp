...
BOOST_AUTO_TEST_CASE(intarg)
{
    ...
    BOOST_CHECK_EQUAL(GetArg("-crz", 11), 0);

    ResetArgs("-foo=11 -crz=12");
    BOOST_CHECK_EQUAL(GetArg("-foo", 0), 11);
    BOOST_CHECK_EQUAL(GetArg("-crz", 11), 12);

    ResetArgs("-foo=NaN -crz=NotANumber");
...
BOOST_AUTO_TEST_CASE(doubledash)
{
    ...
    ResetArgs("--foo=verbose --crz=1");
    BOOST_CHECK_EQUAL(GetArg("-foo", ""), "verbose");
    BOOST_CHECK_EQUAL(GetArg("-crz", 0), 1);
}
