...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\cruZado
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\cruZado
    // Mac: ~/Library/Application Support/cruZado
    // Unix: ~/.cruzado
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cruZado";
#else
    fs::path pathRet;
...
{
    // Copied from GetDefaultDataDir and adapter for cruzado params.

    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\cruZadoParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\cruZadoParams
    // Mac: ~/Library/Application Support/cruZadoParams
    // Unix: ~/.cruzado-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cruZadoParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "cruZadoParams";
#else
    // Unix
    return pathRet / ".cruzado-params";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cruzado.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No cruzado.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override cruzado.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cruzadod.pid"));
...
std::string LicenseInfo()
{
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The cruZado Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
