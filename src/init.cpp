...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cruzado.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cruzadod.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-permitcrzemultisig", strprintf(_("Relay non-P2SH multisig (default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 30333, 40333));
...
)
{
    ...
    if (!(
        ...
        // Create the 'cruzado-params' directory or the 'cruZadoParams' folder
        TryCreateDirectory(ZC_GetParamsDir());
    }

    if(!(boost::filesystem::exists(pk_path))) {
        // Download the 'sprout-proving.key' file
        if (!CRZ_FetchParams("https://z.cash/downloads/sprout-proving.key", pk_path.string()))
            return false;
    }

    // Verify the 'sprout-proving.key' file
    if (!(CRZ_VerifyParams(pk_path.string(), "8bc20a7f013b2b58970cddd2e7ea028975c88ae7ceb9259a5344a16bc2c0eef7"))) {
        return false;
    }

    if(!(boost::filesystem::exists(vk_path))) {
        // Download the 'sprout-verifying.key' file
        if (!CRZ_FetchParams("https://z.cash/downloads/sprout-verifying.key", vk_path.string()))
            return false;
    }

    // Verify the 'sprout-verifying.key' file
    if (!(CRZ_VerifyParams(vk_path.string(), "4bd498dae0aacfd8e98dc306338d017d9c08dd0918ead18172bd0aec2fc5df82"))) {
        return false;
    }

    if(!(boost::filesystem::exists(sapling_spend))) {
        // Download the 'sapling-spend.params' file
        if (!CRZ_FetchParams("https://z.cash/downloads/sapling-spend.params", sapling_spend.string()))
            return false;
    }

    // Verify the 'sapling-spend.params' file
    if (!(CRZ_VerifyParams(sapling_spend.string(), "8e48ffd23abb3a5fd9c5589204f32d9c31285a04b78096ba40a79b75677efc13"))) {
        return false;
    }

    if(!(boost::filesystem::exists(sapling_output))) {
        // Download the 'sapling-output.params' file
        if (!CRZ_FetchParams("https://z.cash/downloads/sapling-output.params", sapling_output.string()))
            return false;
    }

    // Verify the 'sapling-output.params' file
    if (!(CRZ_VerifyParams(sapling_output.string(), "2f0ebbcbb9bb0bcffe95a397e7eba89c29eb4dde6191c339db88570e3f3fb0e4"))) {
        return false;
    }

    if(!(boost::filesystem::exists(sprout_groth16))) {
        // Download the 'sprout-groth16.params' file
        if (!CRZ_FetchParams("https://z.cash/downloads/sprout-groth16.params", sprout_groth16.string()))
            return false;
    }

    // Verify the 'sprout-groth16.params' file
    if (!(CRZ_VerifyParams(sprout_groth16.string(), "b685d700c60328498fbde589c8c7c484c722b788b265b72af448a5bf0ee55b50"))) {
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    fIsCrzeMultisigStd = GetBoolArg("-permitcrzemultisig", true);
    ...
    // Initialize cruZado circuit parameters
    if (!(ZC_LoadParams(chainparams)))
        return InitError(_("Error downloading or verifying cruZado network parameters"));
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("cruZado was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild cruZado with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("cruZado was not built with wallet support. Set -mineraddress, or rebuild cruZado with wallet support."));
    }
 #endif // !ENABLE_WALLET
