...
#if defined(NDEBUG)
# error "cruZado cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "cruZado Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason, const CChainParams& chainparams, const int nHeight)
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout) {
        ...
        if (whichType == TX_NULL_DATA)
            nDataOut++;
        else if ((whichType == TX_MULTISIG) && (!fIsCrzeMultisigStd)) {
            reason = "crze-multisig";
            return false;
 ...
 */
bool static DisconnectTip(CValidationState& state, const CChainParams& chainparams, bool fCrze = false)
{
    ...
    const CChainParams& chainparams, CBlockIndex * const pindexPrev)
{
    ...
    // In cruZado this has been enforced since launch, except that the genesis
