...
This implementation of cruZado is not resistant to side-channel attacks. You
should assume (even unprivileged) users who are running on the hardware, or who
are physically near the hardware, that your `cruzadod` process is running on will
be able to:
...
unprivileged) on the hardware your `cruzadod` process runs on until these
