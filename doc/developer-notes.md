...
- [Development guidelines](#development-guidelines)
    - [General cruZado Core](#general-cruzado-project)
    - [Wallet](#wallet)
    - [General C++](#general-c)
 ...
 * need to upgrade. The message is displayed in the status crz.
 * @see GetWarnings()
 */
class CAlert
{
...
Run with the `-testnet` option to run with "play cruzados" on the test network, if you
...
cruZado Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/cruzado-project/cruzado/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_cruzado
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_cruzado --log_level=test_suite
$ valgrind -v --leak-check=full src/cruzadod -printtoconsole
```
...
# A coverage report will now be accessible at `./test_cruzado.coverage/index.html`.
```

**Sanitizers**

cruZado Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 30333.
...
However, in open source software such as cruZado Core, where everyone uses
...
pay attention to for reviewers of cruZado Core code.

General cruZado Core
----------------------
...
- Use explicitly signed or unsigned `char`s, or even better `uint8_t` and
  `int8_t`. Do not use crze `char` unless it is to pass to a third-party API.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: cruZado Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/crz.h` should use the include guard identifier `BITCOIN_FOO_CRZ_H`, e.g.

```c++
#ifndef BITCOIN_FOO_CRZ_H
#define BITCOIN_FOO_CRZ_H
...
#endif // BITCOIN_FOO_CRZ_H
```
...
Some of these are maintained by active developers of cruZado Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against cruZado Core so that they can be integrated
...
file descriptors it will cause problems with cruZado's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to cruZado's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:cruzado-project/cruzado.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cruzado-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cruzado-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of cruZado Core, the wallet was always
