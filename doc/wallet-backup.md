...
Backing up your cruZado private keys is the best way to be proactive about preventing loss of access to your CRZ.
...
These instructions are specific for the officially supported cruZado Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`cruzado.conf` located in the data directory which is `~/.cruzado/` unless it's been overridden with `datadir=` setting):
...
Note that cruzadod will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`cruzado-cli backupwallet <nameofbackup>`.
...
If you generate a new cruZado address, it will not be reflected in the backup file.
...
`cruzado-cli z_exportwallet <nameofbackup>`
...
`cruzado-cli z_importwallet <path/to/exportdir/nameofbackup>`
...
`cruzado-cli z_exportkey <z-address>`
...
`cruzado-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`cruzado-cli z_importkey <z-priv-key>`
...
`cruzado-cli help importprivkey`
